...
function initMap() {
  try {
    ...
    // Cargar y mostrar las geocercas automáticamente
    loadGeofences();
    ...
    showToast("Error al inicializar el mapa: " + error.message, "error");
  }
}
    ...
    apiRequest("getDevices")
      .then((response) => {
        if (response.success) {
          ...
          console.log(
            "[INFO] Datos combinados de dispositivos y API externa:",
            response.data
          );

          // Verificar si hay dispositivos con datos externos
          let devicesWithExternalData = response.data.filter(
            (device) =>
              device.padron !== null ||
          ...
          console.log(
            "[INFO] Dispositivos con datos externos:",
            devicesWithExternalData.length
          );
          if (devicesWithExternalData.length > 0) {
            console.log(
              "[INFO] Muestra de dispositivos con datos externos:",
              devicesWithExternalData.slice(0, 3)
            );
          }
...
function updateDevices(devices) {
  ...
  devices.forEach((device) => {
    if (device.lastPosition) {
      ...
      const isOnline =
        new Date() - new Date(device.lastPosition.deviceTime) < 5 * 60 * 1000;
...
function updateDeviceMarker(device, position) {
  ...
  if (markers[device.id]) {
    ...
    if (window.selectedDeviceId === device.id) {
...
function createMarker(device, position) {
  ...
  marker.on("click", () => {
    window.selectedDeviceId = device.id;
    showVehicleDetails(device, position);
  });

  // Guardar marcador y añadirlo DIRECTAMENTE al mapa
...
function loadGeofences() {
  console.log("🚀 [DEBUG] Iniciando carga de geocercas...");

  // Definir las geocercas estáticas
  const staticGeofences = [
    // Ruta AB
    { name: "01_AB_Proceres", lat: -12.151569, lon: -76.988855, radius: 30 },
    { name: "02_AB_Espinar", lat: -12.114344, lon: -77.036921, radius: 30 },
    { name: "03_AB_EjercitoBrasil", lat: -12.097006, lon: -77.071756, radius: 30 },
    { name: "04_AB_Bolognesi", lat: -12.060921, lon: -77.042050, radius: 30 },
    { name: "05_AB_OvaloLaPaz", lat: -12.040389, lon: -76.998019, radius: 30 },
    { name: "06_AB_Ceres", lat: -12.030840, lon: -76.929611, radius: 30 },
    // Ruta BA
    { name: "01_BA_Chucocaminoreal", lat: -12.041180, lon: -76.976033, radius: 30 },
    { name: "02_BA_Estaciongrau", lat: -12.054154, lon: -77.012864, radius: 30 },
    { name: "03_BA_EjercitoBrasil", lat: -12.097006, lon: -77.071756, radius: 30 },
    { name: "04_BA_Angamosaviacion", lat: -12.111845, lon: -77.000777, radius: 30 },
    { name: "05_BA_Jorgechavez", lat: -12.142627, lon: -76.991186, radius: 30 },
    { name: "06_BA_Paradero", lat: -12.182013, lon: -77.000407, radius: 30 }
  ];
  ...
  fetch(config.apiUrl, {
    ...
    .then((response) => {
      ...
      if (!response.ok) {
        console.error(
          "❌ [DEBUG] Error de red al cargar geocercas:",
          response.status,
          response.statusText
        );
    ...
    .then((data) => {
      ...
      // Si la API devuelve un array vacío, usar las geocercas estáticas
      if (!data.success || !data.data || data.data.length === 0) {
        console.log("ℹ️ [DEBUG] Usando geocercas estáticas ya que la API devolvió un array vacío");
        displayGeofences(staticGeofences);
      } else {
        console.log("✅ [DEBUG] Geocercas cargadas desde la API:", {
    ...
    .catch((error) => {
      console.error("❌ [DEBUG] Error al procesar geocercas:", {
        error: error.message,
        stack: error.stack
      });
      // En caso de error, usar las geocercas estáticas
      console.log("ℹ️ [DEBUG] Usando geocercas estáticas debido a un error en la API");
      displayGeofences(staticGeofences);
    });
}
...
function displayGeofences(geofences) {
  console.log("🎯 [DEBUG] Iniciando displayGeofences con:", {
    cantidad: geofences?.length,
    datos: geofences
  });
  ...
  if (!geofences || !Array.isArray(geofences) || geofences.length === 0) {
    console.log("⚠️ [DEBUG] No hay geocercas para mostrar");
    return;
  }

  console.log("🔄 [DEBUG] Procesando " + geofences.length + " geocercas");

  // Contador de geocercas procesadas correctamente
  let processedCount = 0;

  // Colores para las geocercas según el tipo de ruta
  const geofenceColors = {
    AB: { stroke: "#2196F3", fill: "#2196F3" }, // Azul para ruta AB
    BA: { stroke: "#2196F3", fill: "#2196F3" }  // Azul para ruta BA
  };

  // Procesar cada geocerca
  geofences.forEach((geofence, index) => {
    try {
      console.log(`🔄 [DEBUG] Procesando geocerca ${index + 1}:`, geofence);

      // Determinar el color basado en el nombre de la geocerca
      let color;
      if (geofence.name.includes('_AB_')) {
        color = geofenceColors.AB;
      } else if (geofence.name.includes('_BA_')) {
        color = geofenceColors.BA;
        } else {
        color = geofenceColors.AB; // Color por defecto
      }

      console.log(`🎨 [DEBUG] Color asignado para ${geofence.name}:`, color);

      // Crear círculo con mayor opacidad y borde más grueso
      const layer = L.circle([geofence.lat, geofence.lon], {
      ...
      console.log(`✅ [DEBUG] Círculo creado para ${geofence.name}`);

      // Añadir popup con información mejorada
      layer.bindPopup(`
                <div class="p-3 bg-white rounded-lg shadow-md">
          <h3 class="font-bold text-lg text-gray-800 mb-1">${geofence.name}</h3>
          <div class="text-xs text-gray-500 mt-2">
            <div>Coordenadas: ${geofence.lat.toFixed(6)}, ${geofence.lon.toFixed(6)}</div>
            <div>Radio: ${geofence.radius} metros</div>
          </div>
                </div>
            `);

      // Añadir tooltip mejorado
      ...
      console.log(`✅ [DEBUG] Geocerca ${geofence.name} añadida correctamente`);
    } catch (error) {
      console.error(`❌ [DEBUG] Error al procesar geocerca ${index + 1}:`, {
        error: error.message,
        stack: error.stack,
        geofence: geofence
      });
  ...
  console.log(`📊 [DEBUG] Resumen: ${processedCount} de ${geofences.length} geocercas mostradas correctamente`);

  // Si no se procesó ninguna geocerca, mostrar un mensaje
  if (processedCount === 0) {
    console.warn("⚠️ [DEBUG] No se pudieron mostrar las geocercas");
    showToast("No se pudieron mostrar las geocercas", "warning");
  } else {
    // Hacer zoom a las geocercas si hay alguna
    try {
      console.log("🔍 [DEBUG] Intentando ajustar vista a las geocercas");
      ...
      geofencesLayer.eachLayer((layer) => {
        ...
        // Para círculos que no tienen getBounds pero tienen getLatLng y getRadius
          ...
          // Ajustar el zoom para incluir el radio completo
      ...
      if (bounds.isValid()) {
        console.log("✅ [DEBUG] Ajustando vista a las geocercas");
        map.fitBounds(bounds, { padding: [50, 50] });
      } else {
        console.warn("⚠️ [DEBUG] No se pudo crear bounds válidos para las geocercas");
      }
    } catch (error) {
      console.error("❌ [DEBUG] Error al ajustar vista a las geocercas:", {
        error: error.message,
        stack: error.stack
      });
    }
...
function loadRoutes() {
  ...
  fetch("rutas.json")
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          "No se pudo cargar el archivo de rutas desde /public/rutas.json"
        );
      }
      return response.json();
    })
...
function displayRoute(routeData, routeType) {
  ...
  for (let i = 0; i < routeData.length; i++) {
    ...
    if (
      ...
      if (
        ...
        if (
          i < 3 ||
          i >= routeData.length - 3 ||
          i % Math.floor(routeData.length / 5) === 0
        ) {
          /* console.log eliminado */ `Punto ${i}: [${point.latitud}, ${point.longitud}]`;
        }
      } else {
        console.error(
          `Punto ${i} fuera de rango: [${point.latitud}, ${point.longitud}]`
        );
      }
    } else {
  ...
  if (routePoints.length < 2) {
    console.error(
      `No hay suficientes puntos válidos para crear la ruta de ${routeType}`
    );
    return;
  }
...
function addRoutesLegend() {
  // Función vacía - No mostrar leyenda de rutas
  // Si existe una leyenda anterior, eliminarla
  if (window.rutasLegend) {
    window.rutasLegend.remove();
    window.rutasLegend = null;
  }
  // No creamos nueva leyenda
  return;
}
...
function showVehicleDetails(device, position) {
  ...
  detailsElement.innerHTML = `
        ...
        <div class="flex flex-col border rounded-lg p-2 bg-base-200">
            ...
            <div class="text-xs flex items-center gap-1">
                ...
                <span class="font-medium">${position.latitude.toFixed(
                  6
                )}, ${position.longitude.toFixed(6)}</span>
            </div>
            <div class="text-xs flex items-center gap-1" id="panel-address-${
              device.id
            }">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
                ...
                <span>Dirección:</span>
                <button class="btn btn-xs btn-primary btn-outline gap-1 show-address" data-lat="${
                  position.latitude
                }" data-lon="${position.longitude}" data-device-id="${
    device.id
  }">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
  ...
  if (engineStopButton) {
    ...
    engineStopButton.parentNode.replaceChild(
      newEngineStopButton,
      engineStopButton
    );
...
function loadDeviceRoute(deviceId) {
  ...
  if (!window.deviceData || !window.deviceData[deviceId]) {
    console.error(
      `[DEBUG] loadDeviceRoute - Error: No se encontró el dispositivo con ID ${deviceId} en deviceData`
    );
    /* console.log eliminado */ "[DEBUG] deviceData disponible:",
      window.deviceData;
  ...
  if (deviceNameElement) {
    ...
    console.warn(
      "[DEBUG] loadDeviceRoute - Elemento route-device-name no encontrado"
    );
  }
...
function loadRouteWithDates(deviceId, fromDate, toDate) {
  ...
  if (fromDate > now) {
    console.warn(
      `[DEBUG] loadRouteWithDates - ADVERTENCIA: La fecha 'from' está en el futuro`
    );
    useAlternativeDates = true;
  }
  if (toDate > now) {
    console.warn(
      `[DEBUG] loadRouteWithDates - ADVERTENCIA: La fecha 'to' está en el futuro`
    );
    useAlternativeDates = true;
  }
  ...
  const intervalHours = Math.floor(
    (intervalMs % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
  );
  const intervalMinutes = Math.floor(
    (intervalMs % (1000 * 60 * 60)) / (1000 * 60)
  );
  ...
  if (window.deviceData && window.deviceData[deviceId]) {
    ...
    console.warn(
      `[DEBUG] loadRouteWithDates - ADVERTENCIA: No se encontró el dispositivo ${deviceId} en datos locales`
    );
  }
  ...
  map.eachLayer((layer) => {
    if (
      ...
      layer.getElement() &&
      layer.getElement().querySelector(".route-marker")
    ) {
  ...
  apiRequest("getRoute", {
    ...
    .then((response) => {
      /* console.log eliminado */ `[DEBUG] loadRouteWithDates - Respuesta recibida:`,
        response;

      if (response.success && response.data && response.data.length > 0) {
        /* console.log eliminado */ `[DEBUG] loadRouteWithDates - Puntos recibidos: ${response.data.length}`;
        /* console.log eliminado */ `[DEBUG] loadRouteWithDates - Primer punto:`,
          response.data[0];
        /* console.log eliminado */ `[DEBUG] loadRouteWithDates - Último punto:`,
        ...
        if (filteredData.length === 0) {
          console.error(
            `[DEBUG] loadRouteWithDates - Error: No se encontraron puntos para el dispositivo ${deviceId}`
          );
          ...
          showToast(
            `No se encontraron puntos para el dispositivo seleccionado`,
            "warning"
          );
          return;
        }
        ...
        showToast(
          `Mostrando ruta con ${filteredData.length} puntos`,
          "success"
        );
      } else {
        console.warn(
          `[DEBUG] loadRouteWithDates - No hay datos disponibles:`,
          response
        );
        // Mostrar un mensaje más detallado
        const message =
          "No hay datos de ruta disponibles para el período seleccionado. " +
        ...
        if (deviceId !== testDeviceId) {
          ...
          apiRequest("getRoute", {
            ...
            .then((testResponse) => {
              /* console.log eliminado */ `[DEBUG] loadRouteWithDates - Respuesta de prueba:`,
                testResponse;
              if (
                ...
                /* console.log eliminado */ `[DEBUG] loadRouteWithDates - Primer punto:`,
                  testResponse.data[0];
                /* console.log eliminado */ `[DEBUG] loadRouteWithDates - Último punto:`,
                  testResponse.data[testResponse.data.length - 1];
              } else {
            ...
            .catch((error) => {
              console.error(
                `[DEBUG] loadRouteWithDates - Error en solicitud de prueba:`,
                error
              );
            });
        }
...
function showDeviceRoute(positions) {
  /* console.log eliminado */ `[DEBUG] showDeviceRoute - Iniciando visualización de ruta con ${
  ...
  if (!positions) {
    console.error(
      "[DEBUG] showDeviceRoute - Error: positions es null o undefined"
    );
  ...
  const filteredPositions = positions.filter(
    (pos) => pos.deviceId === firstDeviceId
  );

  if (filteredPositions.length !== positions.length) {
    console.warn(
      `[DEBUG] showDeviceRoute - Se filtraron ${
        positions.length - filteredPositions.length
      } puntos de otros dispositivos`
    );
  ...
  if (positions.length < 2) {
    console.error(
      "[DEBUG] showDeviceRoute - Error: menos de 2 puntos, no se puede dibujar una ruta"
    );
  ...
  /* console.log eliminado */ "[DEBUG] showDeviceRoute - Primer punto:",
    positions[0];
  /* console.log eliminado */ "[DEBUG] showDeviceRoute - Último punto:",
    positions[positions.length - 1];
  /* console.log eliminado */ "[DEBUG] showDeviceRoute - Rango de tiempo:",
    {
      inicio: new Date(positions[0].deviceTime).toLocaleString(),
      fin: new Date(
        positions[positions.length - 1].deviceTime
      ).toLocaleString(),
      duracion:
        Math.round(
          (new Date(positions[positions.length - 1].deviceTime) -
            new Date(positions[0].deviceTime)) /
            60000
        ) + " minutos",
    };

  // Verificar si hay puntos con coordenadas inválidas
  const invalidPoints = positions.filter(
    (point) =>
      !point.latitude ||
      !point.longitude ||
  ...
  if (invalidPoints.length > 0) {
    console.warn(
      `[DEBUG] showDeviceRoute - ADVERTENCIA: Hay ${invalidPoints.length} puntos con coordenadas inválidas`
    );
    /* console.log eliminado */ `[DEBUG] showDeviceRoute - Primer punto inválido:`,
      invalidPoints[0];
  }
  ...
  if (maxDistanceIndex > 0) {
    /* console.log eliminado */ `[DEBUG] showDeviceRoute - Distancia máxima entre puntos: ${(
      maxDistance / 1000
    ).toFixed(2)} km entre los puntos ${
      maxDistanceIndex - 1
    } y ${maxDistanceIndex}`;
  }
  ...
  L.marker([startPosition.latitude, startPosition.longitude], {
    ...
    .addTo(map)
    .bindTooltip(
      "Inicio: " + new Date(startPosition.deviceTime).toLocaleString(),
      {
        permanent: false,
        direction: "top",
      }
  ...
  L.marker([endPosition.latitude, endPosition.longitude], {
    ...
    .addTo(map)
    .bindTooltip("Fin: " + new Date(endPosition.deviceTime).toLocaleString(), {
      permanent: false,
      direction: "top",
    });
  ...
  showToast(
    `Ruta: ${(totalDistance / 1000).toFixed(
      2
    )} km - Duración: ${hours}h ${minutes}m`,
    "info"
  );
}
...
function sendEngineStopCommand(deviceId) {
  // Confirmar acción
  if (
    !confirm("¿Estás seguro de que deseas detener el motor de este vehículo?")
  ) {
    return;
  }
...
function apiRequest(action, params) {
  ...
  return fetch(config.apiUrl, fetchOptions)
    .then((response) => {
      ...
      if (!response.ok) {
        console.error(
          `[DEBUG] apiRequest - Error de red: ${response.status} ${response.statusText}`
        );
        throw new Error("Error de red: " + response.status);
      }
      ...
      clonedResponse.text().then((text) => {
        try {
          if (text.length > 1000) {
            /* console.log eliminado */ `[DEBUG] apiRequest - Respuesta (truncada): ${text.substring(
              0,
              1000
            )}...`;
          } else {
          ...
          /* console.log eliminado */ `[DEBUG] apiRequest - Error al mostrar texto de respuesta:`,
            e;
        }
      });
    ...
    .then((data) => {
      /* console.log eliminado */ `[DEBUG] apiRequest - Datos JSON parseados:`,
        data;
      ...
      if (action === "getRoute") {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          if (data.data.length === 0) {
            console.warn(
              `[DEBUG] apiRequest - ADVERTENCIA: La respuesta de ruta no contiene puntos`
            );
            ...
            if (fromDate > now || toDate > now) {
              console.warn(
                `[DEBUG] apiRequest - ADVERTENCIA: Las fechas están en el futuro, lo que podría explicar la falta de datos`
              );
            }

            // Verificar si el dispositivo existe
            if (window.deviceData && window.deviceData[params.deviceId]) {
              /* console.log eliminado */ `[DEBUG] apiRequest - Dispositivo ${
                params.deviceId
              } existe en datos locales: ${
                window.deviceData[params.deviceId].name
              }`;
            } else {
              console.warn(
                `[DEBUG] apiRequest - ADVERTENCIA: Dispositivo ${params.deviceId} no encontrado en datos locales`
              );
            }
          } else {
            ...
            /* console.log eliminado */ `[DEBUG] apiRequest - Primer punto: deviceId=${
              firstPoint.deviceId
            }, tiempo=${new Date(firstPoint.deviceTime).toLocaleString()}`;
            /* console.log eliminado */ `[DEBUG] apiRequest - Último punto: deviceId=${
              lastPoint.deviceId
            }, tiempo=${new Date(lastPoint.deviceTime).toLocaleString()}`;

            // Verificar si todos los puntos pertenecen al mismo dispositivo
            const deviceIds = [
              ...new Set(data.data.map((point) => point.deviceId)),
            ];
            ...
            if (deviceIds.length > 1) {
              console.warn(
                `[DEBUG] apiRequest - ADVERTENCIA: Los puntos pertenecen a múltiples dispositivos`
              );
            }

            if (!deviceIds.includes(parseInt(params.deviceId))) {
              console.warn(
                `[DEBUG] apiRequest - ADVERTENCIA: Ninguno de los puntos pertenece al dispositivo solicitado ${params.deviceId}`
              );
            }
          }
        } else {
          console.warn(
            `[DEBUG] apiRequest - ADVERTENCIA: Respuesta de ruta no tiene el formato esperado`
          );
        }
      }
...
async function getAddress(lat, lon) {
  try {
    ...
    const response = await fetch(
      `https://us1.locationiq.com/v1/reverse?key=pk.e63c15fb2d66e143a9ffe1a1e9596fb5&lat=${roundedLat}&lon=${roundedLon}&format=json`
    );

    if (!response.ok) {
      if (response.status === 429) {
        throw new Error(
          "Límite de solicitudes excedido. Por favor, inténtalo de nuevo más tarde."
        );
      }
...
window.updateMarkers = function (positions) {
  ...
  if (window.isRouteMode && window.currentRouteDeviceId) {
    positions = positions.filter(
      (position) => position.deviceId === parseInt(window.currentRouteDeviceId)
    );
...
function syncMapTheme() {
  ...
  const locationIQStreets = layers.find(
    (layer) => layer._url && layer._url.includes("locationiq.com/v3/streets")
  );

  const locationIQDark = layers.find(
    (layer) => layer._url && layer._url.includes("locationiq.com/v3/dark")
  );
  ...
  if (isDarkTheme && locationIQDark && !map.hasLayer(locationIQDark)) {
    ...
    !isDarkTheme &&
    locationIQStreets &&
    !map.hasLayer(locationIQStreets)
  ) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  loadDevices().then(() => {
    ...
    if (
      ...
      /* console.log eliminado */ `[DEBUG] Cargando ruta desde parámetros de URL:`,
        routeParams;
      ...
      if (!isNaN(fromDate.getTime()) && !isNaN(toDate.getTime())) {
        ...
        console.error(
          `[DEBUG] Fechas inválidas en los parámetros de URL:`,
          routeParams
        );
...
// En enterRouteMode, modificar para que el marcador seleccionado se añada al mapa
function enterRouteMode(deviceId) {
  ...
  if (markers[deviceId]) {
    ...
    console.warn(
      `[enterRouteMode] No se encontró el marcador para el dispositivo ${deviceId}`
    );
    ...
    if (deviceData[deviceId] && deviceData[deviceId].lastPosition) {
      ...
      console.error(
        `[enterRouteMode] No se encontraron datos para el dispositivo ${deviceId}`
      );
    }
  }
...
// En exitRouteMode, modificar para que todos los marcadores se añadan al mapa
function exitRouteMode() {
  ...
  // Volver a añadir todos los marcadores directamente al mapa
  Object.values(markers).forEach(marker => {
    if (marker && map) {
      marker.addTo(map);
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const permanentGeofencesList = document.getElementById("permanent-geofences-list");
  const toggleGeofencesBtn = document.getElementById("toggle-geofences-btn");
  
  // Manejar el botón de toggle para expandir/minimizar la lista de geocercas
  if (toggleGeofencesBtn) {
    toggleGeofencesBtn.addEventListener("click", function () {
      permanentGeofencesList.classList.toggle("geofences-minimized");
      
      // Cambiar entre los iconos de expandir y colapsar
      const expandIcon = toggleGeofencesBtn.querySelector(".expand-icon");
      const collapseIcon = toggleGeofencesBtn.querySelector(".collapse-icon");
      
      expandIcon.classList.toggle("hidden");
      collapseIcon.classList.toggle("hidden");
      
      if (permanentGeofencesList.classList.contains("geofences-minimized")) {
        console.log("Lista de geocercas minimizada");
      } else {
        console.log("Lista de geocercas expandida");
      }
    });
  }
  ...
  if (loadGeofencesBtn) {
    loadGeofencesBtn.addEventListener("click", function () {
      console.log(
        "%c🌍 GEOCERCAS: Botón de carga de geocercas pulsado",
        "background: #4CAF50; color: white; padding: 2px 5px; border-radius: 3px;"
      );
      showToast("Cargando geocercas...", "info");
      loadGeofences();
      
      // Expandir la lista permanente si está minimizada
      if (permanentGeofencesList.classList.contains("geofences-minimized")) {
        toggleGeofencesBtn.click();
      }
      
      // Mostrar/ocultar lista flotante de geocercas (para compatibilidad con código anterior)
      geofencesList.classList.toggle("hidden");
    });
  }

  // Función común para manejar el clic en una geocerca
  function handleGeofenceClick(element) {
    const lat = parseFloat(element.dataset.lat);
    const lon = parseFloat(element.dataset.lon);
    const name = element.dataset.name;
    
    // Eliminar la clase 'active' de todos los elementos
    document.querySelectorAll('.permanent-geofence-item, .geofence-item').forEach(item => {
    ...
    // Resaltar la geocerca seleccionada
    geofencesLayer.eachLayer(layer => {
      if (layer.getLatLng() && 
    ...
    // Mostrar mensaje
    showToast(`Centrando en ${name}`, "info");
  }

  // Manejar clics en elementos de la lista flotante de geocercas
  document.querySelectorAll('.geofence-item').forEach(item => {
    item.addEventListener('click', function() {
      handleGeofenceClick(this);
    });
  });

  // Manejar clics en elementos de la lista permanente de geocercas
  document.querySelectorAll('.permanent-geofence-item').forEach(item => {
    item.addEventListener('click', function() {
      handleGeofenceClick(this);
    });
  });

  // Cerrar la lista flotante de geocercas al hacer clic fuera
  document.addEventListener('click', function(e) {
    if (!geofencesList.contains(e.target) && !loadGeofencesBtn.contains(e.target)) {
      geofencesList.classList.add('hidden');
    }
  });
  
  // Asegurarse de que las geocercas se carguen al iniciar
  setTimeout(() => {
    loadGeofences();
    
    // Asegurarse de que la lista esté inicialmente minimizada
    if (permanentGeofencesList && !permanentGeofencesList.classList.contains("geofences-minimized")) {
      toggleGeofencesBtn.click();
    }
  }, 1000);
  ...
  document
    .getElementById("btn-load-route")
    .addEventListener("click", function () {
      const deviceId = parseInt(
        document.getElementById("route-device-id").value
      );
  ...
  document
    .getElementById("btn-show-today-route")
    .addEventListener("click", function () {
      const deviceId = parseInt(
        document.getElementById("route-device-id").value
      );
  ...
  document
    .getElementById("btn-show-yesterday-route")
    .addEventListener("click", function () {
      const deviceId = parseInt(
        document.getElementById("route-device-id").value
      );
  ...
  document
    .getElementById("btn-show-week-route")
    .addEventListener("click", function () {
      const deviceId = parseInt(
        document.getElementById("route-device-id").value
      );
  ...
  document
    .getElementById("btn-show-month-route")
    .addEventListener("click", function () {
      const deviceId = parseInt(
        document.getElementById("route-device-id").value
      );
  ...
  // Manejador para el botón de recarga de geocercas
  document.getElementById('reload-geofences-btn').addEventListener('click', function() {
    console.log('Recargando geocercas...');
    loadGeofences();
    });
});
...
document.addEventListener("click", function (e) {
  ...
  if (e.target.closest(".show-address")) {
    ...
    fetch(
      `https://us1.locationiq.com/v1/reverse.php?key=pk.e63c15fb2d66e143a9ffe1a1e9596fb5&lat=${lat}&lon=${lon}&format=json`
    )
      .then((response) => {
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        ${
                          data.address
                            ...
                            ${
                              data.address.road
                                ? `<p><span class="font-medium">Calle:</span> ${data.address.road}</p>`
                                : ""
                            }
                            ${
                              data.address.house_number
                                ? `<p><span class="font-medium">Número:</span> ${data.address.house_number}</p>`
                                : ""
                            }
                            ${
                              data.address.suburb
                                ? `<p><span class="font-medium">Barrio:</span> ${data.address.suburb}</p>`
                                : ""
                            }
                            ${
                              data.address.city || data.address.town
                                ? `<p><span class="font-medium">Ciudad:</span> ${
                                    data.address.city || data.address.town
                                  }</p>`
                                : ""
                            }
                            ${
                              data.address.state
                                ? `<p><span class="font-medium">Estado:</span> ${data.address.state}</p>`
                                : ""
                            }
                            ${
                              data.address.postcode
                                ? `<p><span class="font-medium">C.P.:</span> ${data.address.postcode}</p>`
                                : ""
                            }
                            ${
                              data.address.country
                                ? `<p><span class="font-medium">País:</span> ${data.address.country}</p>`
                                : ""
                            }
                        `
